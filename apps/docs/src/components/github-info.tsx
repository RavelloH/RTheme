...
type GithubInfoProps = AnchorHTMLAttributes<HTMLAnchorElement> & {
  owner: string;
  repo: string;
  token?: string;
  baseUrl?: string;
...
async function getRepoStars(
  ...
  const headers = new Headers({
    ...
    "User-Agent": "NeutralPress-Docs/1.0",
  });
...
export async function GithubInfo({
  ...
  const stars = await getRepoStars(owner, repo, token, baseUrl);
  const starsLabel = stars === null ? "--" : humanizeNumber(stars);
