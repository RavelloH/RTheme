 ...
 * 支持的运行环境
 */
type ResponseEnvironment = "serverless" | "serveraction";
 ...
 * 创建统一格式的 API 响应体
 */
function createResponseBody<T extends ApiResponseData>(
  success: boolean,
  message: string,
  data: T = null as T,
  error?: ApiError,
...
/**
 * 创建统一格式的 API 响应（serverless环境）
 */
function createServerlessResponse<T extends ApiResponseData>(
  status: number,
  success: boolean,
  message: string,
  data: T = null as T,
...
/**
 * 创建统一格式的 API 响应（serveraction环境）
 */
function createServerActionResponse<T extends ApiResponseData>(
  success: boolean,
  message: string,
  data: T = null as T,
  error?: ApiError,
...
class ResponseBuilder {
  ...
  constructor(environment: ResponseEnvironment = "serverless") {
   ...
   * 通用响应函数
   */
  response<T extends ApiResponseData = null>(config: {
    /** HTTP状态码 */
    status?: number;
    /** 是否成功 */
    success?: boolean;
   ...
   * 200 - 成功响应
   */
  ok<T extends ApiResponseData = null>(config?: {
    /** 响应消息 */
    message?: string;
    /** 响应数据 */
    data?: T;
   ...
   * 201 - 创建成功
   */
  created<T extends ApiResponseData>(config?: {
    /** 响应消息 */
    message?: string;
    /** 响应数据 */
    data?: T;
  ...
  badGateway(config?: {
    ...
    return this.response({
      status: 500,
      success: false,
      message: config?.message || "服务器网关错误",
      data: null,
      error: config?.error || {
        code: "INTERNAL_SERVER_ERROR",
        message: "服务器网关错误",
   ...
   * 带缓存的成功响应
   */
  cached<T extends ApiResponseData>(config: {
    /** 响应消息 */
    message?: string;
    /** 响应数据 */
    data?: T;
...
// 导出工具函数和类型
export { createSecurityHeaders, createCacheHeaders, createPaginationMeta };
