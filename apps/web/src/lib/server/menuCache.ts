 ...
 * 按分类获取菜单项
 */
export async function getMenusByCategory(category: 'MAIN' | 'COMMON' | 'OUTSITE'): Promise<MenuItem[]> {
 ...
 * 按分类获取活跃的菜单项
 */
export async function getActiveMenusByCategory(category: 'MAIN' | 'COMMON' | 'OUTSITE'): Promise<MenuItem[]> {
  const allMenus = await getMenus()
  return allMenus.filter(menu => menu.category === category && menu.status === 'ACTIVE')
}