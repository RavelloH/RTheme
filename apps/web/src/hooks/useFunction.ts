...
export const useFunction = <T extends FunctionMap = FunctionMap>() =>
  create<FunctionState<T>>((set, get) => ({
    ...
    callFunction: async <K extends keyof T>(
      name: K,
      ...args: Parameters<T[K]>
    ): Promise<ReturnType<T[K]>> => {
      const { functions } = get();
      ...
      try {
        ...
        return result instanceof Promise
          ? result
          : Promise.resolve(result as ReturnType<T[K]>);
      } catch (error) {
